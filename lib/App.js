...
App.prototype._finishInit = function() {
  ...
  console.log('~~~~_finishInit afterDataTime', afterDataTime, '' + ((afterDataTime - beforeDataTime) / 1000) + 's');

  this.model.createConnection(data);

  var afterModelTime = Date.now();
  console.log('~~~~_finishInit afterModelTime', afterModelTime, '' + ((afterModelTime - afterDataTime) / 1000) + 's');
