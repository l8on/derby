...
App.prototype._finishInit = function() {
  ...
  var beforeDataTime = Date.now();
  console.log('~~~~_finishInit beforeDataTime', new Date(beforeDataTime));

  var data = JSON.parse(script.nextSibling.textContent);

  var afterDataTime = Date.now();
  console.log('~~~~_finishInit afterDataTime', new Date(afterDataTime), '' + ((afterDataTime - beforeDataTime) / 1000) + 's');

  this.model.createConnection(data);

  var afterModelTime = Date.now();
  console.log('~~~~_finishInit afterModelTime', new Date(afterModelTime), '' + ((afterModelTime - afterDataTime) / 1000) + 's');
...
App.prototype._contentReady = function() {
  ...
  function onDOMContentLoaded(e) {
    console.log('~~~~_contentReady onDOMContentLoaded', e);
